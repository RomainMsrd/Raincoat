...
class cross_domain_trainer(object):
   ...
   This class contain the main training functions for our AdAtime
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                if self.da_method == 'RAINCOAT':
                    ...
                    for epoch in range(1, self.hparams["num_epochs"] + 1):
                        ...
                        algorithm.train()
                        for step, ((src_x, src_y, _), (trg_x, _, trg_index)) in joint_loaders:
                            src_x, src_y, trg_x, trg_index = src_x.float().to(self.device), src_y.long().to(
                                self.device), \
                                trg_x.float().to(self.device), trg_index.to(self.device)
    ...
    def learn_t(self, d1, d2):
        ...
        for i in range(6):
            ...
            if cc.shape[0] > 3:
                ...
                print(i, dip)
                if dip < 0.05:
                    kmeans = KMeans(n_clusters=2, random_state=0, max_iter=5000, n_init=50, init="random").fit(
    ...
    def save_result(self, df, name):
        ...
        result = pd.concat(
            objs=(iDF.set_index('scenario') for iDF in (mean_acc, mean_f1, mean_H, std_acc, std_f1, std_H)),
